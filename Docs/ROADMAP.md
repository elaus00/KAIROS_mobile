...
| 기능 | 범위 | 서브페이즈 |
|------|------|-----------|
| DB 스키마 | v12→v13 (`parent_capture_id`, `body` 추가) | 2b-0 |
| 멀티 인텐트 | AI 분류 시 다중 의도 → 개별 Capture 분할 | 2b-1 |
| 노트 편집 | NoteDetailScreen (제목/본문/태그/폴더 편집) | 2b-2 |
| 폴더 검증 | 이름 최대 30자 제한 | 2b-3 |
| 히스토리 필터 | 분류 유형 칩 + 날짜 범위 필터 | 2b-4 |
| 검색 필터 | FTS 검색 + 유형/날짜 필터 | 2b-5 |
| 할일 위젯 | 4x2 위젯, 오늘 마감 할일 + 체크박스 | 2b-6 |
| 관측 지표 | 4개 신규 이벤트 (revisited, completed, search, split) | 2b-7 |
| 테스트 | 신규/변경 UseCase·ViewModel 테스트 | 2b-8 |

**예상 규모**: ~52개 파일 (18 신규, 34 수정)

**개발 계획**: `Docs/plan/phase2b/implementation_plan.md`

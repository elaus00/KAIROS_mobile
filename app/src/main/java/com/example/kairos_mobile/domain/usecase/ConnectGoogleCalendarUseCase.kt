...
/**
 * M11: Google Calendar 연동 UseCase
 * Google OAuth 인증 및 연동 관리
 */
@Singleton
class ConnectGoogleCalendarUseCase @Inject constructor(
    ...
    suspend fun getAuthUrl(): Result<String> {
        return repository.getGoogleAuthUrl()
    }
    ...
    suspend fun handleCallback(code: String, state: String?): Result<Unit> {
        return repository.handleGoogleCallback(code, state)
    }
    ...
    suspend fun sync(): Result<SyncResult> {
        return repository.triggerGoogleSync()
    }
    ...
    suspend fun disconnect(): Result<Unit> {
        return repository.disconnectGoogle()
    }
}

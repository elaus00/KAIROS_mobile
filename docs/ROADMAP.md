...
## 브랜드 아이덴티티 구현 — 완료 (2026-02-14)

**목표**: Flit. 브랜드 로고, 앱 아이콘, 스플래시, 워드마크를 전체 앱에 적용

| 항목 | 범위 | 상태 |
|------|------|------|
| 로고 확정 | Sora 480 워드마크 + 독립 채움 도트, IC-5a 앱 아이콘 | **완료** |
| 앱 아이콘 | Adaptive Icon (라이트/다크 동적 전환 + 모노크롬) | **완료** |
| 스플래시 | Compose 브랜드 스플래시 (워드마크 + 태그라인, 페이드 애니메이션) | **완료** |
| 워드마크 적용 | 홈 탑바, 온보딩, 로그인, 위젯 — FlitWordmark 컴포넌트 | **완료** |
| 에셋 패키지 | Play Store 512px PNG, 프레스킷 SVG/PNG, Feature Graphic, 소셜 프로필 | **완료** |

**구현 규모**: ~15개 파일 (9 신규, 6 수정)

**명세**: `Docs/specs/brand-identity.md` v2.1
